...
public class BnB {
    ...
    public void Algorithm() {
        ...
        while(visited.size()!=size) { //jesli lista odwiedzonych wierzcholkow nie zawiera wszystkich wierzcholkow
            FindNext(); //znalezienie nastepnego grafu (znajduje index nastepnego grafu (o najniższym koszcie), który bedzie przeszukiwany)
            ...
            for (int i = 0; i < graphs.get(0).size; i++) {
                if (!visited.contains(i)) { //jesli lista odwiedzonych grafow zawiera dany wierzcholek nie dodawaj nowego grafu
                    graphs.add(new Graphs(graphs.get(index).size, graphs.get(index).graph, graphs.get(index).cost, graphs.get(index).path));
                    //kopiowanie grafu poczatkowego do rozpatrywania jego polaczen
                    graphs.get(graphs.size() - 1).CalculatingCost(visited.get(visited.size() - 1), i);
                    //obliczanie sciezki przejscia z danego wierzcholka do jego nieodwiedzonych polaczen
